...
@Component({
  ...
  template: `
    <section class="mt-12">
      <h2 tocHeading>Command Dialog</h2>
      <p class="mb-6 text-muted-foreground">
        A command dialog that opens in a modal. Perfect for command palettes, search interfaces, and
        quick actions.
      </p>
...
export class CommandDialogDemoSection {
  ...
  protected dialogDemoCode = `<!-- Trigger Button -->
<button
  sc-button
  variant="outline"
  class="justify-start"
...
<!-- Command Dialog -->
@if (isDialogOpen()) {
  <div sc-dialog class="fixed inset-0 z-50">
    <div class="fixed inset-0 bg-black/50" (click)="closeDialog()"></div>
    <div sc-dialog-content class="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg">
      <sc-command-dialog>
        <sc-command-input placeholder="Type a command or search..." />
        <sc-command-list>
          <sc-command-empty>No results found.</sc-command-empty>
