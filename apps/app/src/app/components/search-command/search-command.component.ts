...
@Component({
  ...
  template: `
    ...
    >
      <sc-command class="rounded-lg border shadow-md bg-background max-w-2xl w-full mx-4 max-h-96">
        <sc-command-input
          class="h-14"
          [value]="searchQuery()"
...
export class SearchCommandComponent implements OnInit, OnDestroy {
  ...
  // Icons for template (no longer needed with individual icon components)
  ...
  private clearSearch(): void {
    ...
    this.searchResults.set([]);
    this.algoliaService.clearSearch();
  }
