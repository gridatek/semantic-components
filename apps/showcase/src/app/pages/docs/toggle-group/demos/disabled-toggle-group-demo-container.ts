...
@Component({
  ...
  template: \`
    <div class="flex flex-col gap-4">
      <div scToggleGroup type="single" [disabled]="true" aria-label="Disabled toggle group">
      ...
      <div scToggleGroup type="single" aria-label="Toggle group with disabled item">
        <button scToggleGroupItem value="a" aria-label="Option A">A</button>
        <button scToggleGroupItem value="b" [disabled]="true" aria-label="Option B (disabled)">B</button>
        <button scToggleGroupItem value="c" aria-label="Option C">C</button>
      </div>
    </div>
  \`,
