...
The rating component supports half-star increments (0.5 steps) when `[allowHalf]="true"` is set on `scRating`. This enables:
...
Colors are controlled via CSS variables on `ScRating`:
