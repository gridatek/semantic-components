...
@Component({
  selector: 'sc-autocomplete-panel',
  imports: [ScAutocompleteOption],
  template: `
    <div
      class="autocomplete-panel relative z-50 max-h-60 min-w-[8rem] overflow-auto rounded-md border bg-popover p-1 text-popover-foreground shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2"
      [id]="listboxId()"
      ...
      @if (!isLoading() && grouped() && filteredItems().length > 0) {
        @for (group of getGroups(); track group.name) {
          ...
          @for (item of group.items; track item.value; let i = $index) {
            <div
              class="autocomplete-option relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-2 pr-8 text-sm outline-none aria-selected:bg-accent aria-selected:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50"
              [item]="item"
              [isSelected]="isItemSelected(item)"
              [id]="'option-' + group.name + '-' + i"
              (click)="selectItem(item)"
              (mouseenter)="setActiveItem(item)"
              scAutocompleteOption
            >
...
export class ScAutocompletePanel {
  @ViewChildren(ScAutocompleteOption) options!: QueryList<ScAutocompleteOption>;

  readonly listboxId = input<string>('');
  readonly filteredItems = input<(string | ScAutocompleteItem)[]>([]);
  ...
  readonly itemSelected = output<string | ScAutocompleteItem>();
  readonly itemActiveChange = output<string | ScAutocompleteItem>();
  ...
  isItemSelected(item: string | ScAutocompleteItem): boolean {
  ...
  getItemLabel(item: string | ScAutocompleteItem): string {
  ...
  getGroups(): { name: string; items: ScAutocompleteItem[] }[] {
    const groups: { [key: string]: ScAutocompleteItem[] } = {};
