...
export class ScLightboxProvider {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (completed >= 1 && !this.lightbox.isOpen()) {
  ...
  private getOverlayRef() {
    if (!this.overlayRef) {
      ...
      this.overlayRef.backdropClick().subscribe(() => this.lightbox.close());
      this.overlayRef.keydownEvents().subscribe((event) => {
        if (event.key === 'Escape') {
          if (this.lightbox.closeOnEscape()) {
            this.lightbox.close();
          }
        } else if (event.key === '+' || event.key === '=') {
          this.lightbox.zoomIn();
        } else if (event.key === '-') {
          this.lightbox.zoomOut();
        } else if (event.key === '0') {
          this.lightbox.resetZoom();
        }
      });
    }
